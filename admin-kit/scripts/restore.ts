...
const ORDER = ["setting", "businessHour", "location", "product", "blogPost", "coupon", "review", "gallery", "galleryImage", "vipMember", "newsletterSub", "order", "staffMember", "staffPunch", "staffTask", "jobApplication"];
...
(async () => {
  ...
  for (const t of ORDER) {
    ...
    const pk = t === "setting" ? "key" : t === "businessHour" ? "day" : "id";
    for (const r of rows) {
      const { [pk]: id, ...rest } = r;
